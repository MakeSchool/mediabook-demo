...
Try creating one inline in your mdx. Write

- the prompt
- the answer
- optionally, a SelfCheck
...
You can also use props for all these:

<FlashCard
  prompt={'How do you assign a variable in python? Draw it out.'}
  answer={'You use the assignment operator, ='}
  selfcheck={<SelfCheck />}
/>
...
4. `answer` for the text that will show after submit.

There is an answer. It doesn't get passed as a prop - it's just whatever text appears after the
ordered list.
...
<TextResponse prompt={'What excites you about learning to code?'} answer={'Thanks for sharing.'} />
...
An empty `prompt` prop will render a check without a prompt (this text is in the markdown above the
component)

<ConfidenceCheck prompt="" />

<ConfidenceCheck>How are you feeling about shortcodes?</ConfidenceCheck>
